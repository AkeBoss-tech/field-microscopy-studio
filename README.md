...
The **Channels** menu offers selected-channel color, grayscale, or an additive combined-channel view in 2D and sampled 3D. Each channel has editable color, brightness, contrast, local-background display subtraction, and composite visibility, saved per dataset in this browser. These settings change the viewport only; processing, source pixels, masks, measurements and scientific TIFF exports retain their original values. The active channel always remains visible and is the only channel used for processing, annotation, and 3D picking. iMOP defaults to green/blue; unnamed channels use display defaults, not inferred stain identities or recovered acquisition lookup tables.

In Explore, **Z method** switches among maximum, arithmetic average and sum of the acquired planes. Previously the source projection was a maximum; additive compositing of multiple visible channels can also look bright. **Save view** downloads the rendered viewport as a PNG, including its colors and overlays. **3D region** accepts native X/Y/Z start and exclusive end coordinates, initially using the visible XY field, then samples only that region. **Full field** resets the crop. From Review, select a full-volume mask candidate and choose **Open cell in 3D** to inspect sampled signal inside its current corrected mask. This is an isolated candidate preview, not a biological cell identification or a complete surface mesh. Saved 2D projection runs have no source Z stack for recomputing average or sum; select the raw source for those views.
