...
The **Channels** menu offers selected-channel color, grayscale, or a combined-channel view in 2D and 3D. Each channel has editable color, brightness, contrast, local-background display subtraction, and composite visibility, saved per dataset in this browser. The interactive histogram controls black and white display points after automatic source-to-display scaling; it is not an absolute raw-intensity histogram. These settings change the viewport only; processing, source pixels, masks, measurements and scientific TIFF exports retain their original values. The active channel always remains visible and is the only channel used for processing, annotation, and 3D picking. iMOP defaults to green/blue; four unnamed hair-cell channels use magenta/orange/green/blue display defaults inspired by the supplied ZEN screenshot, not inferred stain identities or recovered acquisition lookup tables.

In Explore, **Z method** defaults to arithmetic average and can switch to maximum or sum of the acquired planes. **Save view** downloads the rendered viewport as a PNG, including its colors, overlays, optional grid and calibrated scale bar. **3D region** accepts native X/Y/Z start and exclusive end coordinates, initially using the visible XY field, then renders only that region. **Full field** resets the crop. From Review, select a full-volume mask candidate and choose **Open cell in 3D** to inspect signal inside its current corrected mask. This is an isolated candidate preview, not a biological cell identification or a complete surface mesh. Saved 2D projection runs have no source Z stack for recomputing average or sum; select the raw source for those views.
